...
> [`../catalog.yaml`](../catalog.yaml). Conformance rules are defined in
> [`language-conformance.md`](language-conformance.md).
...
local-inference option B, workspace `0.3.8`, latest release `v0.3.7`, and
...
The Atlas/catalog pair owns document routing. This table is the review index;
`yes` means the implementation and evidence were found during the audit.
...
4. `docs/quality/` — conformance, documentation quality, and audit evidence;
5. `docs/planning/` — maintainer plans; completed work is archived by category;
...
`tools/qa/docs_examples.sh` performs the repeatable baseline:
...
`docs/catalog.yaml` is the machine-readable document registry and
`docs/quality/language-conformance.md` defines the feature-to-specification
evidence shape.
`tools/qa/docs_coverage.sh` checks catalog paths and rejects stale canonical
spellings. Future compiler changes should update the catalog/conformance
evidence in the same change as the implementation and regression test; no
second knowledge graph is needed.
