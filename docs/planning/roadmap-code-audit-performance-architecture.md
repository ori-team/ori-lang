...
| **CLI-DAEMON-1** | `pipeline/daemon.rs` keeps the process alive and every request still builds a fresh check/JIT pipeline. | **process-persistent prototype (typed protocol, 2026-09-01)** | request DTOs now use `serde_json`, responses are built structurally, malformed/escaped/invalid-envelope cases, exact shutdown, and bounded line buffering at 1 MiB request/8 MiB source limits are tested; session/cache keyed by source/config/dependencies and warm-vs-cold benchmark remain |
...
| **DX-DOCTEST-1** | The original harness discarded recursive/read/write/temp failures, could fall through from check to JIT, and returned an empty `SourceCache`. | **partial — failure-safe harness (2026-09-01)** | unique per-case temporary directories, all failures become results, cleanup errors are reported, deterministic recursive extraction, source-backed cache, missing-path regression, expected-output assertions and richer source-labelled diagnostics |
