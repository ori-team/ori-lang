...
| **PKG-REG** | Official registry | 4 | L | shelved | Registry v1 (local directory + optional HTTP endpoint) is implemented and tested. A central hosted service, authentication, signing, retention and availability policy require infrastructure outside this compiler repository; keep the item shelved until that service is approved. |
