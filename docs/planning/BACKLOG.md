...
| **LANG-TY-2** | Applicable generic traits | 2 | M | todo | `trait Container[Item]` declares fine, but `apply X use Container[int]` does not parse and bare `use Container` leaves the parameter unbound. **Scoped 2026-07-20**: shares the substitution helper with LANG-TY-3 but needs four more layers of its own — `TraitSig` does not record the trait's type parameters, `ImplSig` does not record the arguments, `ApplyUseSection` has no `trait_args`, and the parser rejects `[` after the trait name |
| **LANG-TY-8** | Receiver-less trait methods | 3 | M | todo | `default() -> Self`, and constructors in general: a trait method with no `self`. Blocks `core.Default` |
