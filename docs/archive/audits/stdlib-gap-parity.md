...
> Status: **archived**  
> Original path: `docs/planning/historico/stdlib-gap-parity.md`  
> Original date: before 2026-07-27  
> Archived on: 2026-07-27  
> Category: `audits`  
> Current replacement: `docs/architecture/stdlib.md and docs/spec/15-stdlib-maintenance.md`  
> Warning: versions, syntax, commands, paths, and priorities below may be obsolete.
...
|------|---------|-----------------|
| Uniformizar Layer 1 FS/io (`bool`/`string` → `result`/`optional`) | ✅ Feito | [`PENDENTES.md`](../../planning/historico/PENDENTES.md) |
...
|------|------------|-----------------|
| `ori explain <code>` | Alta | [`PENDENTES.md`](../../planning/historico/PENDENTES.md) §1 |
| `ori doctor` | Alta | §1 |
| Guia Errors/Null/Void | Alta | §1 |
| `ori repl` | Média | §3 |
| `ori summary` | Média | §3 |
| `if then else` expressão | Média | §3 |
| Registry / installer | Baixa | [`registry-v2.md`](../plans/registry-v2.md) |
...
Recorte inicial entregue: `ori.string`, `ori.list` e `ori.fs` agora têm
módulos pai `.orl` para import seletivo de helpers/algoritmos diretamente no
namespace principal. Os caminhos antigos (`ori.string.utils`, `ori.string.algorithms`,
`ori.list.utils`, `ori.list.algorithms`, `ori.fs.utils`) seguem compatíveis.
Para detalhes de manutenção, consulte `docs/spec/15-stdlib-maintenance.md`
na seção "Namespace flattening (Opção C)".
...
- [`net-v2-design.md`](../investigations/net-v2-design.md) — TLS, servidor TCP, UDP, blocking vs async
- [`PENDENTES.md`](../../planning/historico/PENDENTES.md) — backlog v2 DX + uniformização APIs
